...
with col1:
    st.image(
        "https://marketlytics.com/wp-content/uploads/2023/03/logo-2-1-1024x366-1.png",
        width=2000,
    )
with col2:
    st.markdown(
...
if submitted:
    ...
    with st.status("Running GA4 Audit...", expanded=True) as status:
        try:
            ...
            response = requests.post(AUDIT_ENDPOINT, json=payload, timeout=90)
            response.raise_for_status()
            result = response.json()

            # ---------- EMPTY RESULT HANDLING ---------- #
            if not result:
                status.update(label="⚠️ Insufficient GA4 Data", state="error")
                st.error(
                    "The selected GA4 property does not contain enough data to generate an audit. "
                    "Please ensure sufficient historical data is available and try again."
                )
                st.stop()
            ...
            if not result.get("ecommerce_included", False):
                st.warning(
                    "⚠️ Ecommerce data missing. Skipping ecommerce-related checks.")

            if not result.get("anomaly_included", False):
                st.warning(
                    "⚠️ Revenue anomaly check skipped due to insufficient data.")
            ...
            if buttons_html:
                ...
                st.markdown(
                    "🔗 Click a button above to open or download your audit report.")
            else:
